...
class MatchTeamsData(BaseModel):
    """
    MatchTeamsData
    """ # noqa: E501
    team_aid: Optional[MatchTeamData] = Field(default=None, description="Use querystring param `useSides=true` to change teamId label in `home` or `away`", alias="teamAId")
    team_bid: Optional[MatchTeamData] = Field(default=None, description="Use querystring param `useSides=true` to change teamId label in `home` or `away`", alias="teamBId")
    __properties: ClassVar[List[str]] = ["teamAId", "teamBId"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of team_aid
        if self.team_aid:
            _dict['teamAId'] = self.team_aid.to_dict()
        # override the default output from pydantic by calling `to_dict()` of team_bid
        if self.team_bid:
            _dict['teamBId'] = self.team_bid.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "teamAId": MatchTeamData.from_dict(obj["teamAId"]) if obj.get("teamAId") is not None else None,
            "teamBId": MatchTeamData.from_dict(obj["teamBId"]) if obj.get("teamBId") is not None else None
        })
        return _obj
