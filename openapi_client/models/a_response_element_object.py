...
class AResponseElementObject(BaseModel):
    """
    AResponseElementObject
    """ # noqa: E501
    coaches: Optional[CoachesFetch] = None
    events: Optional[List[MatchEvent]] = None
    formations: Optional[FormationsFetch] = None
    match: Optional[Match] = Field(default=None, description="Available with querystring param `fetch=match`")
    players: Optional[PlayersFetch] = None
    referees: Optional[List[TheRefereeObject]] = Field(default=None, description="Available with querystring param `fetch=referees`")
    substitutions: Optional[SubstitutionFetch] = None
    teams: Optional[TeamsFetch] = None
    __properties: ClassVar[List[str]] = ["coaches", "events", "formations", "match", "players", "referees", "substitutions", "teams"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.events:
            ...
            _dict['events'] = _items
        # override the default output from pydantic by calling `to_dict()` of formations
        if self.formations:
            _dict['formations'] = self.formations.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "coaches": CoachesFetch.from_dict(obj["coaches"]) if obj.get("coaches") is not None else None,
            "events": [MatchEvent.from_dict(_item) for _item in obj["events"]] if obj.get("events") is not None else None,
            "formations": FormationsFetch.from_dict(obj["formations"]) if obj.get("formations") is not None else None,
            "match": Match.from_dict(obj["match"]) if obj.get("match") is not None else None,
            "players": PlayersFetch.from_dict(obj["players"]) if obj.get("players") is not None else None,
            "referees": [TheRefereeObject.from_dict(_item) for _item in obj["referees"]] if obj.get("referees") is not None else None,
            "substitutions": SubstitutionFetch.from_dict(obj["substitutions"]) if obj.get("substitutions") is not None else None,
            "teams": TeamsFetch.from_dict(obj["teams"]) if obj.get("teams") is not None else None
        })
        return _obj
