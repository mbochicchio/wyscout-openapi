...
class TheSubstitutionThatHappenedAtStartSecSecondsElapsedFromTheStart(BaseModel):
    """
    TheSubstitutionThatHappenedAtStartSecSecondsElapsedFromTheStart
    """ # noqa: E501
    var_in: Optional[List[TheEntranceOfAPlayerAsASubstitute]] = Field(default=None, alias="in")
    out: Optional[List[TheExitOfAPlayerFromTheFieldRedCardsIncluded]] = None
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.var_in:
            ...
            _dict['in'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in out (list)
        _items = []
        if self.out:
            for _item_out in self.out:
                if _item_out:
                    _items.append(_item_out.to_dict())
            _dict['out'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "in": [TheEntranceOfAPlayerAsASubstitute.from_dict(_item) for _item in obj["in"]] if obj.get("in") is not None else None,
            "out": [TheExitOfAPlayerFromTheFieldRedCardsIncluded.from_dict(_item) for _item in obj["out"]] if obj.get("out") is not None else None
        })
        return _obj
